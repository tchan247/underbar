(function() {
  ...
  _.reduce = function(collection, iterator, accumulator) {
  	...
  	var memo, i, len = collection.length;
  ...
  _.every = function(collection, iterator) {
    ...
    iterator = iterator || function(x) {return x};	// default iterator if not provided

    return _.reduce(collection, function(allMatch, item){
    	if(!iterator(item)) allMatch = false;
    	return allMatch;
    }, true);
  };
  ...
  _.defaults = function(obj) {
  	...
  	for(var i=1, len = arguments.length; i<len; i++) {
  		for(var k in arguments[i]) {
  			if(!(k in obj)) obj[k] = arguments[i][k];
  		}
  	}
  ...
  _.memoize = function(func) {
  	...
  	var memo = {};	// store stringified arguments as keys to the results

  	return function() {

  		var args = Array.prototype.slice.apply(arguments).toString();
  ...
  _.intersection = function() {
    ...
    for(var i=0, len=arrays.length; i<len; i++) {
      // see if the element exists in other arrays
      for(var j=1; j<len; j++) {
        if(_.indexOf(arrays[j], arrays[0][i]) !== -1
          && _.indexOf(result, arrays[0][i] === -1)) {  //make sure element is unique in results
  ...
  _.difference = function(array) {
    ...
    // check each element in array argument
    for(var i=0, len=array.length; i<len; i++) {
      var found = false;

      // search other arrays for current element and stop if element is found
      for(var j=0, len2=others.length; j<len2 && !found; j++) {
        if(_.indexOf(others[j], array[i]) !== -1) {
          found = true;
        }
      }
